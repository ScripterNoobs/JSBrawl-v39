...
class ByteStream {
  ...
  /**
   * Writing value to Bytes as String using VarInt prefix
   * @param {String} value Your value to write.
   */
  ...
  /**
   * Writing values to Bytes as two Ints
   * @param {Number} value1
   * @param {Number} value2
   */
  ...
  /**
   * Writing value to Bytes as Zlib compressed String
   * @param {String} value
   */
  writeCompressedString (value) {
    ...
    this.ensureCapacity(4)
    this.buffer[this.offset++] = (buf.length)
    this.buffer[this.offset++] = (buf.length >> 8)
    this.buffer[this.offset++] = (buf.length >> 16)
    this.buffer[this.offset++] = (buf.length >> 24)

    this.buffer = Buffer.concat([this.buffer, compressed])
